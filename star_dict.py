...
# star_dict.py -
#
# Modified by XFY9326 on 2020/01/19
# Last Modified: 2020/01/19 23:39
#
# ======================================================================
...
class StarDict(object):
    ...
    def match(self, word, limit=10, strip=False):
        ...
        if not strip:
            ...
            c.execute(sql, (strip_word(word), limit))
        records = c.fetchall()
        result = []
    ...
    def register(self, word, items, commit=True):
        ...
        try:
            self.__conn.execute(sql, (word, strip_word(word)))
...
class DictMySQL(object):
    ...
    def query_batch(self, keys):
        ...
        for key in keys:
            if isinstance(key, int) or isinstance(key, long):
                query_list.append('id = %s')
            elif key is not None:
                query_list.append('word = %s')
        sql = sql + ' or '.join(query_list) + ';'
        query_word = {}
        query_id = {}
        with self.__conn as c:
    ...
    def register(self, word, items, commit=True):
        ...
        try:
            with self.__conn as c:
                c.execute(sql, (word, strip_word(word)))
...
class DictCsv(object):
    ...
    def __init__(self, filename, codec='utf-8'):
        self.__csv_name = None
        if filename is not None:
            self.__csv_name = os.path.abspath(filename)
        self.__codec = codec
    ...
    def __read(self):
        self.reset()
        filename = self.__csv_name
        if filename is None:
            return False
        if not os.path.exists(self.__csv_name):
            return False
        codec = self.__codec
        if sys.version_info[0] < 3:
            fp = open(filename, 'rb')
        ...
        for row in reader:
            ...
            if word in words:
                continue
            row.extend([0, 0, strip_word(row[0])])
            words[word] = 1
            rows.append(row)
            index.append(row)
        self.__rows = rows
        self.__index = index
        self.__rows.sort(key=lambda _row: _row[0].lower())
        self.__index.sort(key=lambda _row: (_row[COLUMN_SW], _row[0].lower()))
    ...
    def save(self, filename=None, codec='utf-8'):
        ...
        for row in self.__rows:
            ...
            for n in row:
                ...
                new_row.append(n)
            writer.writerow(new_row[:COLUMN_SIZE])
        fp.close()
        return True
    ...
    def __obj_decode(self, row):
        if row is None:
            return None
        obj = {'id': row[COLUMN_ID], 'sw': row[COLUMN_SW]}
        skip = self.__numbers
    ...
    def update(self, key, items):
        ...
        if row is None:
            return False
        new_row = self.__obj_encode(items)
        for name, idx in self.__fields:
            if idx == 0:
                continue
            if name in items:
    ...
    def commit(self):
        if self.__csv_name:
            self.save(self.__csv_name, self.__codec)
        return True
...
class DictHelper(object):
    ...
    def __init__(self):
        self._exchanges = {'p': u'过去式', 'd': u'过去分词', 'i': u'现在分词', '3': u'第三人称单数', 'r': u'比较级', 't': u'最高级',
                           's': u'复数', '0': u'原型', '1': u'类别'}
        self._pos = {'a': (u'代词', 'pron.'), 'c': (u'连接词', 'conj.'), 'd': (u'限定词', 'determiner'), 'i': (u'介词', 'prep.'),
                     'j': (u'形容词', 'adj.'), 'm': (u'数词', 'num.'), 'n': (u'名词', 'n.'), 'p': (u'代词', 'pron.'),
                     'r': (u'副词', 'adv.'), 'u': (u'感叹词', 'int.'), 't': (u'不定式标记', 'infm.'), 'v': (u'动词', 'v.'),
                     'x': (u'否定标记', 'not')}
    ...
    @staticmethod
    def dump_map(dictionary, lower=True):
        words = {}
    ...
    # 字典差异导出
    def discrepancy_export(self, dictionary, words, out_name, opts=''):
        existence = self.dump_map(dictionary)
        if os.path.splitext(out_name)[-1].lower() in ('.txt', '.csv'):
            _db = DictCsv(out_name)
        else:
            _db = StarDict(out_name)
        _db.delete_all()
        count = 0
        for word in words:
            ...
            try:
                ...
                continue
            _db.register(word, {'tag': 'PENDING'}, False)
            count += 1
        _db.commit()
    ...
    def discrepancy_import(self, dictionary, filename, opts=''):
        ...
        if os.path.splitext(filename)[-1].lower() in ('.csv', '.txt'):
            _db = DictCsv(filename)
        else:
            _db = StarDict(filename)
        count = 0
        for word in self.dump_map(_db, False):
            data = _db[word]
            if data is None:
                continue
            if data['tag'] != 'OK':
    ...
    def deficit_tab_txt(self, dictionary, txt, out_name, opts=''):
        deficit = {}
        for line in codecs.open(txt, encoding='utf-8'):
            ...
            word = row[0]
            deficit[word] = 1
        return self.discrepancy_export(dictionary, deficit, out_name, opts)

    # 导出星际译王的词典文件，根据一个单词到释义的字典
    def export_stardict(self, word_map, out_name, title):
        main_name = os.path.splitext(out_name)[0]
        keys = [k for k in word_map]
        keys.sort(key=lambda x: (x.lower(), x))
        import struct
        pc = self.progress(len(word_map))
        position = 0
        with open(main_name + '.idx', 'wb') as f1:
            with open(main_name + '.dict', 'wb') as f2:
                for word in keys:
                    pc.next()
                    f1.write(word.encode('utf-8', 'ignore') + b'\x00')
                    text = word_map[word].encode('utf-8', 'ignore')
                    f1.write(struct.pack('>II', position, len(text)))
                    f2.write(text)
                    position += len(text)
            with open(main_name + '.ifo', 'wb') as f3:
                f3.write(b'StarDict\'s dict ifo file\nversion=2.4.2\n')
                f3.write(b'wordcount=%d\n' % len(word_map))
                f3.write(b'idxfilesize=%d\n' % f1.tell())
    ...
    # 导出 mdict 的源文件
    def export_mdict(self, word_map, out_name):
        keys = [k for k in word_map]
        keys.sort(key=lambda x: x.lower())
        size = len(keys)
        index = 0
        pc = self.progress(size)
        with codecs.open(out_name, 'w', encoding='utf-8') as fp:
            for key in keys:
                pc.next()
                word = key.replace('</>', '').replace('\n', ' ')
                text = word_map[key].replace('</>', '')
    ...
    # 导入mdx源文件
    @staticmethod
    def import_mdict(filename, encoding='utf-8'):
        import codecs
        words = {}
    ...
    @staticmethod
    def export_mdx(word_map, out_name, title, desc=None):
        try:
            import writemdict
        except ImportError:
        ...
        writer = writemdict.MDictWriter(word_map, title=title,
                                        description=desc)
    ...
    @staticmethod
    def read_mdx(mdx_name, mdd=False):
        try:
            import readmdict
        except ImportError:
        ...
        words = {}
        if not mdd:
            mdx = readmdict.MDX(mdx_name)
        else:
            mdx = readmdict.MDD(mdx_name)
    ...
    # 读取词形变换字符串
    @staticmethod
    def exchange_loads(exchange):
        if not exchange:
            return None
        obj = {}
        for text in exchange.split('/'):
            pos = text.find(':')
            if pos < 0:
                continue
            k = text[:pos].strip()
    ...
    # 设置详细内容，None代表删除
    @staticmethod
    def set_detail(dictionary, word, item, value, create=False):
    ...
    # 取得详细内容
    @staticmethod
    def get_detail(dictionary, word, item):
        data = dictionary.query(word)
        if not data:
            return None
        detail = data.get('detail')
    ...
    @staticmethod
    def load_text(filename, encoding=None):
        try:
            content = open(filename, 'rb').read()
        except:
            return None
    ...
    # 保存 tab 分割的 txt文件
    @staticmethod
    def tab_txt_save(filename, words, encoding='utf-8'):
    ...
    def mdx_build(self, src_name, out_name, title, desc=None):
        print('loading %s' % src_name)
        t = time.time()
        words = self.import_mdict(src_name)
        t = time.time() - t
        print(u'%d records loaded in %.3f seconds' % (len(words), t))
        print(u'building %s' % out_name)
        t = time.time()
        self.export_mdx(words, out_name, title, desc)
        t = time.time() - t
...
# 字典转化，csv sqlite之间互转
def convert_dict(dst_name, src_name):
    dst = open_dict(dst_name)
    src = open_dict(src_name)
    dst.delete_all()
...
def open_local(filename):
    ...
    for _dir in [base, base + '/share', base + '/share/stardict']:
